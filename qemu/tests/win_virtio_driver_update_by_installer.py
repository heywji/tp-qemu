...
def run(test, params, env):
    ...
    def run_installer_serial(session_serial, run_installer_cmd):
        """
        Run the virtio-win installer via serial session only,
        without depending on network login at any point.

        After AutoIt finishes, unconditionally reboot via the
        framework's vm.reboot (serial=True) which correctly
        handles serial console reconnection.

        :param session_serial: serial console session
        :param run_installer_cmd: installer command string
        :return: a fresh serial session after reboot
        """
        run_installer_cmd = utils_misc.set_winutils_letter(
            session_serial, run_installer_cmd
        )
        vm.send_key("meta_l-d")
        time.sleep(5)
        win_driver_utils.copy_file_to_samepath(session_serial, test, params)
        test.log.info("Running installer via serial: %s", run_installer_cmd)
        session_serial.sendline(run_installer_cmd)
        ...
        def _autoit_done():
            try:
                return session_serial.cmd_status(autoit_check_cmd) != 0
            except (
        ...
        if not utils_misc.wait_for(_autoit_done, 600, 5, 5):
            test.fail("AutoIt installer still running after 600s")

        test.log.info("AutoIt finished, checking if VM rebooted...")
        vm_rebooted = utils_misc.wait_for(
            lambda: not session_serial.is_responsive(), 120, 5, 5
        )
        if vm_rebooted:
            test.log.info("VM rebooted by installer, waiting for VM to come back")
            if not utils_misc.wait_for(vm.is_alive, 300, 5, 5):
                test.fail("VM not alive after installer reboot")
            test.log.info("VM is alive, waiting before system_reset")
            time.sleep(60)
        else:
            test.log.info("VM did not reboot by installer")

        test.log.info("Performing system_reset to get clean serial console")
        ...
        try:
            out = session_serial.cmd_output("echo SERIAL_OK", timeout=30)
            test.log.info("Serial console verified: %s", out.strip())
        except Exception:
            test.log.info("Serial echo failed, trying another system_reset")
            vm.monitor.cmd("system_reset")
            session_serial = vm.wait_for_serial_login(
                timeout=480,
                recreate_serial_console=True,
    ...
    def get_nic_ifname(session_serial):
        """
        Get NIC connection name using PowerShell (works reliably
        on serial console where wmic often hangs).

        :param session_serial: serial session
        :return: NIC connection name string
        """
        mac_for_ps = virtio_nic_mac.replace(":", "-")
        ps_cmd = (
            'powershell -command "'
            "Get-NetAdapter | Where-Object {$_.MacAddress -eq '%s'} "
            '| Select-Object -ExpandProperty Name"' % mac_for_ps
        )

        def _get():
            try:
                output = session_serial.cmd_output(ps_cmd, timeout=60)
                name = output.strip()
                if name and "Get-NetAdapter" not in name:
                    return name
            except Exception as e:
                test.log.info("NIC ifname query failed (will retry): %s", e)
            return None
        ...
        result = utils_misc.wait_for(
            ...
            step=15,
            first=10,
            text="Waiting for NIC ifname to be ready",
        )
        if not result:
            test.fail("Failed to get NIC ifname for MAC %s" % virtio_nic_mac)
        return result
    ...
    def get_nic_guid(session_serial):
        ...
        ps_cmd = (
            'powershell -command "'
            "(Get-NetAdapter | Where-Object {$_.MacAddress -eq '%s'}).InterfaceGuid"
            '"' % mac_for_ps
        )
        ...
        def _get_guid():
            try:
                ...
                test.log.info("GUID query failed (will retry): %s", e)
            return None

        setting_id = utils_misc.wait_for(
            _get_guid,
            timeout=120,
            step=10,
            first=5,
            text="Waiting for NIC GUID",
        )
        if not setting_id:
            test.fail("Failed to get valid NIC GUID for MAC %s" % virtio_nic_mac)
    ...
    def read_registry_baseline(session_serial):
        ...
        mac_for_dns = virtio_nic_mac.replace(":", "-")
        dns_ps_cmd = (
            'powershell -command "'
            "$idx = (Get-NetAdapter | Where-Object "
            "{$_.MacAddress -eq '%s'}).ifIndex; "
            "(Get-DnsClientServerAddress -InterfaceIndex $idx "
            "-AddressFamily IPv4).ServerAddresses -join ','\"" % mac_for_dns
        )
        try:
            dns_str = session_serial.cmd_output(dns_ps_cmd, timeout=60).strip()
        except Exception:
            dns_str = ""
        test.log.info("Baseline DNS servers: %s", dns_str)

        mac_for_gw = virtio_nic_mac.replace(":", "-")
        gw_ps_cmd = (
            'powershell -command "'
            "$idx = (Get-NetAdapter | Where-Object "
            "{$_.MacAddress -eq '%s'}).ifIndex; "
            "(Get-NetIPConfiguration -InterfaceIndex $idx)"
            '.IPv4DefaultGateway.NextHop"' % mac_for_gw
        )
        try:
            gateway = session_serial.cmd_output(gw_ps_cmd, timeout=60).strip()
    ...
    def check_network_config(session_serial, baseline):
        ...
        mac_for_check = virtio_nic_mac.replace(":", "-")
        dns_ps = (
            'powershell -command "'
            "$idx = (Get-NetAdapter | Where-Object "
            "{$_.MacAddress -eq '%s'}).ifIndex; "
            "(Get-DnsClientServerAddress -InterfaceIndex $idx "
            "-AddressFamily IPv4).ServerAddresses -join ','\"" % mac_for_check
        )
        try:
            post_dns = session_serial.cmd_output(dns_ps, timeout=60).strip()
        except Exception:
            post_dns = ""
        ...
        gw_ps = (
            ...
            "{$_.MacAddress -eq '%s'}).ifIndex; "
            "(Get-NetIPConfiguration -InterfaceIndex $idx)"
            '.IPv4DefaultGateway.NextHop"' % mac_for_check
        )
        try:
            post_gateway = session_serial.cmd_output(gw_ps, timeout=60).strip()
            if not re.match(r"\d+\.\d+\.\d+\.\d+", post_gateway):
                post_gateway = None
        except Exception:
            post_gateway = None
    ...
    def install_heavyload(session_serial):
        """Install HeavyLoad stress tool in the guest."""
        virtio_letter = utils_misc.get_winutils_vol(session_serial)
        install_cmd = params["heavyload_install_cmd"] % virtio_letter
        test.log.info("Installing HeavyLoad: %s", install_cmd)
        session_serial.cmd_status(install_cmd, timeout=120)
        install_path = params["heavyload_install_path"]
        check = session_serial.cmd_status('dir "%s\\HeavyLoad.exe"' % install_path)
        if check != 0:
            test.fail("HeavyLoad not found after install at %s" % install_path)
        test.log.info("HeavyLoad installed at %s", install_path)

    def start_heavyload(session_serial):
        """Start HeavyLoad stress in the background."""
        install_path = params["heavyload_install_path"]
        cpu = params.get("heavyload_cpu", 2)
        mem = params.get("heavyload_memory", 50)
        fio = params.get("heavyload_file", 50)
        dur = params.get("heavyload_duration", 0)
        cmd = (
            'start "" /B "%s\\HeavyLoad.exe" '
            "/CPU %s /MEMORY %s /FILE %s /DURATION %s /AUTOEXIT /START"
            % (install_path, cpu, mem, fio, dur)
        )
        test.log.info("Starting HeavyLoad: %s", cmd)
        session_serial.sendline(cmd)
        time.sleep(5)
        status = session_serial.cmd_status("tasklist | findstr /i HeavyLoad.exe")
        if status == 0:
            test.log.info("HeavyLoad is running")
        else:
            test.log.info("HeavyLoad may not have started (non-critical)")

    def stop_heavyload(session_serial):
        """Kill HeavyLoad process if running."""
        test.log.info("Stopping HeavyLoad")
        session_serial.cmd_status("taskkill /f /im HeavyLoad.exe")
    ...
    if params.get("update_from_previous_installer", "no") == "yes":
        ...
        session = run_installer_serial(session, run_install_cmd)
    else:
    ...
    heavyload_enabled = params.get("heavyload_stress", "no") == "yes"
    if heavyload_enabled:
        error_context.context("Install and start HeavyLoad stress", test.log.info)
        install_heavyload(session)
        start_heavyload(session)

    try:
        error_context.context(
            "Set up static IP/DNS on virtio NIC before upgrade", test.log.info
        )
        network_baseline = setup_network_config(session)

        try:
            error_context.context("Run viofs service...")
            virtio_fs_utils.run_viofs_service(test, params, session)
        except Exception as e:
            test.log.info("Ignoring viofs service error: %s", e)
        ...
        if heavyload_enabled:
            test.log.info("Re-starting HeavyLoad after installer reboot")
            start_heavyload(session)
        ...
        if network_baseline:
            ...
            check_network_config(session, network_baseline)
            error_context.context(
                "Restore network config to DHCP after upgrade check",
                test.log.info,
            )
            restore_network_config(session)
    ...
    try:
        ...
        test.log.info("Ignoring gagent version check error (serial/wmic): %s", e)

    try:
        win_driver_installer_test.driver_check(session, test, params)
    except Exception as e:
        test.log.info("Ignoring driver check error (serial/wmic): %s", e)
    ...
    try:
        ...
        if fail_tests:
            test.log.info(
                "Driver function tests had failures (non-critical): %s",
                fail_tests,
            )
    except Exception as e:
        test.log.info(
            "Ignoring driver function test error (network unavailable): %s", e
        )
